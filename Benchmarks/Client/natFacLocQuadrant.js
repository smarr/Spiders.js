...
function mHandle(event){
	...
	function customerMsg(sender,pointString){
		...
		if(children == null){
			...
			if((childrenBoundaries.length == 4) && (childSpawned == 4)){
				var index = 0
				while(index <= 3){
	...
	function partition(){
		...
		postMessage(["spawnQuad",0,"TOP_LEFT",firstBoundary.x1,firstBoundary.y1,firstBoundary.x2,firstBoundary.y2,threshold,depth + 1,knownFacilities,maxDepthOpenFac],[chan1.port1])
		var chan2 = new MessageChannel()
		chan2.port2.onmessage = mHandle
		postMessage(["spawnQuad",1,"TOP_RIGHT",secondBoundary.x1,secondBoundary.y1,secondBoundary.x2,secondBoundary.y2,threshold,depth + 1,knownFacilities,maxDepthOpenFac],[chan2.port1])
		var chan3 = new MessageChannel()
		chan3.port2.onmessage = mHandle
		postMessage(["spawnQuad",2,"BOT_LEFT",thirdBoundary.x1,thirdBoundary.y1,thirdBoundary.x2,thirdBoundary.y2,threshold,depth + 1,knownFacilities,maxDepthOpenFac],[chan3.port1])
		var chan4 = new MessageChannel()
		chan4.port2.onmessage = mHandle
		postMessage(["spawnQuad",3,"BOT_RIGHT",fourthBoundary.x1,fourthBoundary.y1,fourthBoundary.x2,fourthBoundary.y2,threshold,depth + 1,knownFacilities,maxDepthOpenFac],[chan4.port1])
	}
