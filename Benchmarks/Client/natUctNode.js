...
function mHandle(event){
	...
	function generateChildren(currentId,compSize){
		...
		while(i < binomial){
			...
			postMessage(["spawnNodeP1",childrenHeight,idValue + 1,compSize,false,partId],[rootCop.port1])
			var c2 = new MessageChannel()
			c2.port2.onmessage = mHandle
			postMessage(["spawnNodeP2",partId],[c2.port1])
			partId += 1
			i += 1
		}
		hasChildren = true
	...
	function generateUrgentChildren(urgentChildId, currentId, compSize){
		...
		while(i < binomial){
			var rootCop = new MessageChannel()
			root.postMessage(["link"],[rootCop.port2])
			postMessage(["spawnNodeP1",childrenHeight,idValue + 1,compSize,i == urgentChildId,partId],[rootCop.port1])
			var c2 = new MessageChannel()
			c2.port2.onmessage = mHandle
			postMessage(["spawnNodeP2",partId],[c2.port1])
			partId += 1
