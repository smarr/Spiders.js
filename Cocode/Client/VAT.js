...
$(document).ready(function () {
    $.getJSON("http://jsonip.com/?callback=?", function (data) {
        ...
        if(exportCallback != null){
        	console.log(data.ip)
        	exportCallback(actor,systemActor,futureManager.onResolve,futureManager.onRuin,killAll)
        }
    });
});
...
function sanitise(object){
	...
	for(var property in object)
	{
		if(!(serialiseModule.isFarRef(object[property])) && !(serialiseModule.isProxy(object[property]))){
			retObject[property] = object[property]
		}
	}
	return retObject
...
function makePreLoadBehaviour(worker,id,behaviour){
	return function(e){
		if(msgModule.isLoadedMsg(e.data)){
			...
			//Problem is bigger, could be that system behaviour has a far ref as instance variable at the time of actor spawning 
			worker.postMessage(msgModule.makeInjectMsg(serialiseModule.serialise(behaviour),serialiseModule.serialise(sanitise(systemBehaviour))))
		}
...
return function(cb){
	if(systemIp != null){
	    cb(actor,systemActor,futureManager.onResolve,futureManager.onRuin,killAll);
	} else {
	    exportCallback = cb;
	}
}
